...
const xmlUrlNode = (domain, pagePath, postMetadata) => {
  ...
  if (isPost) {
    const meta = postMetadata.filter(post => pagePath.endsWith(post.slug));
    // meta will be empty if this is a draft post.
    // doesn't affect production but this would cause a build to fail in dev
    if (meta) {
      date = new Date(meta[0].dateModified || meta[0].date);
    }
  }
  return `<url>
  <loc>${url}</loc>
...
const generateSitemap = async (
  ...
  const sitemap = xmlUrlWrapper(
    pages.map(page => xmlUrlNode(domain, page, postMetadata)).join('\n')
  );
